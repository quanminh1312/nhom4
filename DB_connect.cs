    ...
    internal class DB_connect : DbContext
    {
        ...
        public async Task<List<Phim>> ReadPhimsortdirector(string director)
        {
            using (var context = new DB_connect())
            {
                ...
                // Lấy danh sách các sản phẩm trong bảng 
                var phims = await (from p in context.Phims
                                   where (p.Director.Contains(director))
                                   select p
                                 )
                                .ToListAsync();
                return phims;
        ...
        public async Task<List<Phim>> ReadPhimsortcountry(string country)
        {
            using (var context = new DB_connect())
            {
                ...
                // Lấy danh sách các sản phẩm trong bảng 
                var phims = await (from p in context.Phims
                                   where (string.Compare(p.Country, country) == 0)
                                   select p
                                 )
                                .ToListAsync();
                return phims;
        ...
        public async Task updatePhim(string id)
        {
            using (var context = new DB_connect())
            {
                ...
                if (phim != null)
                {
                    ...
                    {
                        string year;
                        Console.WriteLine("Ten phim cu: " + phim.Name);
                        Console.Write("Ten phim moi : ");
                        string name = Console.ReadLine();

                        Console.WriteLine("nam san xuat phim cu: " + phim.Year);
                        Console.Write("Nam san xuat phim moi: ");
                        year = int.Parse(Console.ReadLine()).ToString();

                        Console.WriteLine("dao dien phim cu: " + phim.Director);
                        Console.Write("dao dien phim moi: ");
                        string director = Console.ReadLine();
                        Console.WriteLine("nuoc san xuat phim cu: " + phim.Country);
                        Console.Write("nuoc san xuat Phim (ma nuoc 2 ky tu): ");
                        string country = Console.ReadLine();
                        phim.Name = name;
                        phim.Year = year;
                        phim.Director = director;
                        phim.Country = country;
